...
**Re-checked Wednesday, September 9, 10:00 AM Eastern (R):** the same page read **"Only accepting scheduled orders."** Kevin's read, same day: the kitchen opens at 11 with the taproom, so scheduled-only an hour before opening is Toast behaving correctly, and the morning visit is not evidence of anything. Agreed; the proposal states it as expected behavior. The finding rests on the Thursday-evening visit, mid-service, when the button offered nothing. The re-check did surface a second fault: **the Toast menu and the website menu disagree.** Soup is a cup 6 / bowl 9 on `/brewery` and cup $5 / bowl $8 on Toast; Dark Onion Soup 9 versus $8; Toast lists Hot Dog ($2), Coney Dog ($3), Chips & Dip ($8) and a $15.18 "Sivarticus" that appear nowhere on the site. Pizza and sandwich prices match. The proposal's claim card was rewritten to carry both visits and the price mismatch.
...
- **No age gate.** Everyone is welcome at the brewery, and the 655KB script that never showed one goes away with nothing in its place. (Ruled by Kevin 2026-09-09; the proposal's build table says the same.)
...
**The price melts from a published anchor (Kevin, 2026-09-09: "like the other proposals").** Markup and script lifted verbatim from the Sprinkles pitch. The anchor is Untappd's own guide, ["How Much Should a Website Cost in 2025? A Guide for Restaurants, Bars, and Breweries"](https://lounge.untappd.com/how-much-should-a-website-cost/) (Untappd Team, Feb 6, 2025): fully custom website **$10,000 to $20,000+ upfront and $100 to $500 a month**; mid-range $1,000 to $5,000 plus $50 to $200. We count down from $10,000, the bottom of the fully custom band, to $4,500; the monthly card cites the $100 to $500 band next to $195. Chosen over the brewery estimator (freelancer $3,650 to $10,900, agency $8,700 to $26,100) because Untappd is a company they already pay and the guide is theirs. Per `proposal.md`: `data-to` is the real price in the DOM, the anchor lives only in `data-from`, reduced motion and blocked scripts see $4,500. The source line under the cards says what the anchor buys and which column we are.

**The closing paragraph under the findings** (Kevin, 2026-09-09) reframes the nine as a matter of attention rather than bugs and states what the studio specializes in: attention to detail, the experience of the person using the site, and whether the client is happy; "we believe in good businesses and see it as our mission to help them grow." It says "when the details are nobody's job," which is about the arrangement, and must stay that way. An earlier "some of these are a five-minute edit" note was removed the same day at Kevin's request.

**Launch window: two to four weeks** (Kevin, 2026-09-09), stated in section three under the build table and again in step four, "depending on how much we customize and how quickly the conversation settles it." Was four to six.

**The audience is Emily** (Kevin, 2026-09-09). Her address is the one their Mug Club page gives for renewals; **she does not manage the website, someone else does** (Kevin, same day). **Nothing in the proposal may point at a person**, her, whoever manages the site, or anyone else: faults are the platform's and the arrangement's. Every "one employee", "one inbox", "one mailbox" and her name were removed the same day and reframed to the arrangement: no online way to join, renew, or get release news. The "if she is out for a month" question was deleted, not softened. The hero reads "Four other companies do." When editing, grep the proposal for `emily|inbox|mailbox|employee|one person` before a send; the only legitimate hit is "one person empowered to say yes."
...
| `demo/site.css`, `demo/site.js` | One stylesheet, one script. Everything in the script is an enhancement: with JavaScript off every page is complete. The script also carries the motion layer (scroll reveals below the hero, a 26-second hero drift, a float on the can strip, a pulse on the open dot; transform and opacity only; all of it off under reduced motion) and the live tap swap. |
| `demo/taps.mjs`, `api/taps.js` | **The live tap list.** Their Untappd for Business board page is server-rendered with the whole board as JSON in a `data-react-props` attribute; `taps.mjs` reads and parses it. `build.mjs` bakes the latest board into the pages (the no-JavaScript fallback is as fresh as the last build), and `/api/taps` (a Vercel function, cached five minutes at the edge) serves it live; `site.js` swaps it in on the home and menu pages. On the real build this is the Untappd for Business API with their token, same fields, no page parsing. `package.json` exists only to mark the repo ESM for the function. |
| `tools/trim-cans.mjs` | Trims every can shot to its alpha bounds and re-exports at 540 tall (Chromium canvas through the local server), because the source PNGs carry different transparent margins and six cans in a row rendered six sizes. 46 cans, 19 to 57KB each as lossy WebP with alpha. |
| `demo/assets/` | 79 files, about 3MB. Pulled from Wix's CDN at layout sizes: marks, posters and the logo as AVIF with alpha (the same file as WebP came down at 192KB, as AVIF 44KB); cans as trimmed WebP (above); photos as WebP; the one PNG-sourced photo as JPEG. `demo/assets/og.jpg` is the demo's own link card, the client's, rendered from `demo/og-card.html`. |
...
**What the demo does that the proposal promised:** `/demo/menu` is the menu (taproom, pizza, breakfast, tap list, each anchored, with `Menu` schema); the homepage shows all three venues' status computed from the Marshall clock (at 9:45 PM Thursday it read "Taproom open now · kitchen until 9 PM", "Commons opens Friday 7 AM", "General Store opens Friday 12 PM"); events show a weekday that comes from the date, "Free, no ticket needed", and an add-to-calendar file, and past ones remove themselves; the Mug Club page has renew and December-list flows; there is no age gate, by Kevin's ruling of 2026-09-09 (everyone is welcome at the brewery; their 655KB script never showed anyone anything); contact has `tel:`, `mailto:`, directions, all three hours blocks, and a map behind a click; the Instagram section is six photos, not eight video players; the tap list is live from their Untappd board through `/api/taps`, with the last build's board baked in as the fallback; the five brand pages are one system with each beer a card. Ordering still links to Toast's URL (the hostname move needs their account) and there is no workroom, both as the proposal says.

**Measured (`tools/render-demo.mjs`, 2026-09-03):** 28 page renders, 0 errors, 0 overflow, 0 broken images, 0 4xx. Homepage 954KB desktop / 843KB phone after the can trim (trimmed lossy WebP weighs a little more than the CDN's AVIF did), 29 requests, all first-party including `/api/taps` (no web fonts; system stack). Contact 60KB (the Google Maps embed is a facade until asked for; it was 2.1MB with the iframe live). Brand pages 340 to 660KB because every can is on the page.
...
- The tap list is live (see `demo/taps.mjs` above). Kevin asked 2026-09-09 whether it was wired up; it was a labeled snapshot until then. Their board's menu was last edited by them on 2026-09-05, so they do keep Untappd current, which is what makes this worth doing.
- Merch links out to InkSoft; no product images were reachable. **What InkSoft is** (Kevin asked 2026-09-09): a storefront platform sold to screen-print and embroidery shops (part of Inktavo, with Printavo). A store at `stores.inksoft.com/<name>` lives under a print shop's account, here publisher #20305, whose name the storefront markup does not expose. The shop hosts the store, decorates the goods (on demand or in small batches) and fulfills; the brand gets a margin and never holds inventory. So yes: functionally a decorator-run print-on-demand store, run by whoever their local printer is. Worth asking Chuck who that is; it decides whether merch can ever come in-house.

  **What in-house could mean (Kevin's sidenote, 2026-09-09).** If the printer fulfills, which is the normal InkSoft arrangement, an in-house store has three shapes, and only one of them is clean:
  1. **Stocked goods, fulfilled by them.** The General Store already sells merch over the counter, so whatever they physically stock (shirts, hats, glassware) can go on our store today, shipped or held for pickup from the General Store, with our checkout and their margin intact. This is the clean one and it needs nothing from the printer.
  2. **Printer's on-demand catalog behind our storefront.** InkSoft has no order-injection path for a third-party storefront that I could find (unverified; the printer may know otherwise), so this would mean forwarding orders to the print shop by hand or email. Workable at their volume, not elegant.
  3. **Swap the printer for an API print-on-demand service.** Clean technically, but it fires the local printer, and that is Chuck's call, not a website decision.
  The pitch line stays as written in the build table: shirts on their pages, in their brand, checkout with the print shop until they want it in-house. Three questions for Chuck settle which shape: who the printer is, what merch the General Store actually holds in stock, and what the split is.
...
- **From the table-top menus in the taproom (photos from Kevin, 2026-09-09), none of it on their website:** DH Gin cocktails (Spiked Raspberry Lemonade 8, Mojito 8 in five flavors, Gin & Juice 7, Gin & Tonic 7) and DH Vodka cocktails (Super Duper White Russian 8, Dark Mule 8, Cape Cod 7, Pom Fizz 7, Bloody Mary 9, Vanilla Coke 7), both spirits distilled and bottled by Dark Horse in Marshall at 90 proof with bottles sold at the Commons and General Store; Crooked Tree Cellars wine with Fennville Winery (Meritage 7, Cabernet Sauvignon 7, Chardonnay 5, Sauvignon Blanc 5, Blackberry Sangria 8, any glass a spritzer); a third dessert, Oatmeal Stout Chocolate Cake 8; the Commons breakfast card's exact side and drink prices (grapefruit juice, 12 oz cold brew 5, toast choices). The demo's menu page now has Cocktails and Wine sections with `Menu` schema, and the homepage's tap block points at them. **The distillery finally has a place on the site.** The printed taproom beer menu (dated Sept 5) matches the Untappd board the live tap list reads, 14 pours, pour for pour.
- **"Sivarticus" is real.** The printed Cheese Bread line reads "make it a Sivarticus: add jalapeños and bacon +4." The website menu has the add-on without the name, so Toast's $15.18 "Sivarticus" is cheese bread with the works, not a phantom. Removed from the proposal's Toast sentence the same day; hot dogs and the Coney stay, since neither appears on any menu, printed or online. Cheese Bread in the demo now carries the name.
...
- **Whether Toast online ordering is switched off deliberately in the evenings.** Still one evening observation (Thu Sep 3, 7:15 PM). The Sep 9 morning visit does not count either way: before the 11 AM opening, scheduled-only is correct. A second evening check during service would settle whether it is a pattern; the proposal presents it as one dated instance and does not call it a pattern.
...
| `tools/serve.mjs`, `tools/render-demo.mjs`, `tools/trim-cans.mjs` | Local server mirroring `vercel.json` (including `/api/taps`), the demo's render walk, and the can trimmer |
| `api/taps.js`, `demo/taps.mjs`, `package.json` | The live tap list function and its parser; the package file marks the repo ESM |
